...
function robotCreator(robot) {
	...
	robot.joints.forEach(joint => {
		...
		if (parentLinkInRobot == null) {
			...
			if (parentLink.visual.geometry.type == 'mesh') {
				let parentLinkModel = createLinkModel(parentLink);
				
			} else {
				
			}

			console.log(parentLinkModel);
			console.log(parentLinkModel.children[0]);
		...
		if (childLinkInRobot == null) {
			...
			let childLinkModel = createLinkModel(childLink);

			console.log(childLinkModel);
			console.log(childLinkModel.children[0]);
